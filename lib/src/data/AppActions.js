...
const AppActions = {
	addDiv(location) {
		WeatherApi.getWeatherObject(location).then(data => {
			...
			var initTime = dayFormat(new Date(list[0].dt_txt)) - 3;
			var temp = 0;
			var hour = 0;
			//Start to fill the return object with weather data
			for(i = 0; i < count; i++){

				var mill = list[i].dt_txt;
				...
				if(i<8){
					dayFormat = d3.timeFormat("%d-%b %H:");
					var date = dayFormat(new Date(mill)) + '';
					var temper = (list[i].main.temp).toFixed(1);
					date = (date + "00 : " + temper + " F");
			...
			dayFormat = d3.timeFormat("%d-%b-%y");
			var obj = {	day : '', count : '', };
			obj.day = dayFormat(new Date(list[i-1].dt_txt));
			if(obj.day === result.graphList[3].day){
				obj.day = dayFormat(new Date(list[i-1].dt * 1000 + (86400000)));
			}
			obj.count = temp/((i%8)+1);
			result.graphList.push(obj);
